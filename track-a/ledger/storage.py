...
def insert_invoice(db, row):
    ...
    if old:
        ...
        raise ValueError(
            'Invoice identity already exists with different amount or due date'
        )
...
def insert_payment(db, row, invoice_id):
    ...
    if old:
        if (
            old['customer_id'] == row['customer_id']
            and old['invoice_number'] == row['invoice_number']
            and old['amount'] == row['amount']
        ):
            return 'skipped'

        raise ValueError(
            'Payment ID already exists with different details'
        )
